...
def parse_args_n_config():
    ...
    if (config['maddpg']['restore'] or config['maddpg']['display'] or config['maddpg']['benchmark']) and config['maddpg']['load_dir'] == "":
        config['maddpg']['load_dir'] = load_dir
    if (config['maddpg']['restore'] or config['maddpg']['display'] or config['maddpg']['benchmark']) and config['maddpg']['save_dir'] == "":
        config['maddpg']['save_dir'] = load_dir
...
def make_env(arglist, config, show=False):
    if config['domain']['factorization'] == '4x2':
...
def get_trainers(env, num_adversaries, obs_shape_n, action_shape_n, config, arglist):
    ...
    for i in range(num_adversaries, len(env.possible_agents)):
        trainers.append(trainer(
            "agent_%d" % i, model, obs_shape_n, action_shape_n, i, arglist,
